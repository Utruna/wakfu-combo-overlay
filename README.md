...
Les 3 **sorts communs** (Maîtrise d'Armes, Charme de Masse, Os à Moelle — débloqués par quête, utilisables par n'importe quelle classe) suivent le même traitement : absents des pages classe, ids figés en dur dans `COMMON_QUEST_SPELLS`, sourcés depuis la base de données de stratfu.fr et vérifiés contre le CDN Ankama. Ils sont stockés sous une pseudo-classe `"commun"` dans `data/spellIcons.json` — la recherche d'icône (`findSpellIcon` dans `electron/main.js`) retombe automatiquement dessus pour n'importe quel héros, quelle que soit sa classe.
